...
describe PickReviewers do
  let(:available_tag) { Plankbot::Tag.create(name: "Yes", kind: "availability") }
  let(:not_available_tag) { Plankbot::Tag.create(name: "No", kind: "availability") }
  let(:time_1_tag) { Plankbot::Tag.create(name: "06:00-15:00", kind: "time_available") }
  let(:time_2_tag) { Plankbot::Tag.create(name: "07:00-16:00", kind: "time_available") }
  let(:time_3_tag) { Plankbot::Tag.create(name: "08:00-17:00", kind: "time_available") }
  let(:time_4_tag) { Plankbot::Tag.create(name: "09:00-18:00", kind: "time_available") }
  let(:onboarding_tag) { Plankbot::Tag.create(name: "onboarding", kind: "scrum") }
  let(:min_tag) { Plankbot::Tag.create(name: "min", kind: "scrum") }
  let(:mout_tag) { Plankbot::Tag.create(name: "mout", kind: "scrum") }
  let(:prodeng_tag) { Plankbot::Tag.create(name: "prodeng", kind: "scrum") }
  ...
  let(:review_ready_label) do
    Plankbot::Label.create(name: "review_ready", reviewer_count: 2)
  end

  let(:onboarding_label) do
    Plankbot::Label.create(name: "onboarding", reviewer_count: 1)
  end

  let(:min_label) do
    Plankbot::Label.create(name: "min", reviewer_count: 2)
  end
  ...
  describe PickTeammate do
    specify do
      ...
      non_teammate_reviewers =
        ctx[:chosen].select{|x| x.tags.where(kind: "scrum").where.not(name: onboarding_tag.name).exists? }
    ...
    it "chooses reviewers with least assignments" do
      ...
      ctx[:chosen].each do |c|
        ctx[:remaining].each do |r|
          binding.pry unless c.pull_request_count <= r.pull_request_count
